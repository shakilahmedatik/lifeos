...
export interface RssFeedRepository {
  ...
  update(id: string, patch: Partial<NewRssFeedInput>, userId?: string): Promise<RssFeed | undefined>;
  updateStatus(id: string, status: "active" | "inactive", userId?: string): Promise<RssFeed | undefined>;
  updateFetchStatus(
    id: string,
...
export interface NewsArticleRepository {
  ...
  getByFeedId(feedId: string, userId?: string, limit?: number, offset?: number): Promise<NewsArticle[]>;
  getRecent(limit: number, userId?: string): Promise<NewsArticle[]>;
  search(query: string, userId?: string, feedId?: string, limit?: number, offset?: number): Promise<NewsArticle[]>;
